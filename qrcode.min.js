/**
 * qrcode.js - Cross-browser QR Code generator in JavaScript
 * MIT License
 */
var QRCode;
(function () {
	...
	QRCodeModel.prototype = {
		...
		createMovieClip: function (target_mc, instance_name, depth) {
			var mc = target_mc.createEmptyMovieClip(instance_name, depth);
			var minGrid = 1;
			this.make();
			for (var row = 0; row < this.modules.length; row++) {
				...
				for (var col = 0; col < this.modules[row].length; col++) {
					...
					if (dark) {
						mc.beginFill(0, 100);
						mc.moveTo(x, y);
						mc.lineTo(x + minGrid, y);
						mc.lineTo(x + minGrid, y + minGrid);
						mc.lineTo(x, y + minGrid);
						mc.endFill();
					}
				}
			}
	...
	QRCode.prototype.makeCode = function (sText) {
		...
		for (; type <= 10; type++) {
			try {
				var testQR = new QRCodeModel(type, this._htOption.correctLevel);
				testQR.addData(sText);
				testQR.make();
				this._oQRCode = testQR;
				break;
			} catch (e) {}
		}
		if (type > 10) {
			this._oQRCode = new QRCodeModel(10, QRErrorCorrectLevel.L);
